...
def load_proxies() -> list:
    ...
    """
    proxy_all = []
    HTTP = "greatHttp_ip.txt"
    socks4 = "socks4_proxy.txt"
    socks5 = "socks5_proxy.txt"
    proxy_file = [HTTP,socks4,socks5]
...
def load_json() -> List[API]:
    ...
    :return: api list
    """
    json_path = pathlib.Path(path, 'api_spare.json')
...
def run(thread: int, phone: Union[str, tuple], frequency: int, interval: int, enable_proxy: bool = False):
    ...
    with ThreadPoolExecutor(max_workers=thread) as pool:
        for i in range(1, frequency + 1):
            print(i)
            logger.success(f"第{i}波测试开始！")

            # 此處代碼邏輯有問題,如果 _proxy 為空就不會啓動轟炸,必須有東西才行
            for proxy in _proxies:
                logger.success(f"第{i}波测试 - 当前正在使用代理：" +
                               proxy['all://'] + " 进行测试...") if enable_proxy else logger.success(f"第{i}波开始测试...")
                # 不可用的代理或API过多可能会影响测试效果
                for api in _api:
                    if enable_proxy:
                        pool.submit(reqFuncByProxy, api, phone, proxy)
                    else:
                        print(api)
            ...
            logger.success(f"第{i}波测试提交结束！休息{interval}s.....")
            time.sleep(interval)
